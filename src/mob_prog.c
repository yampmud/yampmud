...
const char *fn_keyword[] = {
  ...
  "mobexists",                  /* if mobexists fido - is there a fido
                                   somewhere */
  "objexists",                  /* if objexists sword - is there a sword
                                   somewhere */
  ...
  "clones",                     /* if clones > 3 - are there > 3 mobs of same
                                   vnum here */
  ...
  "hastarget",                  /* if hastarget $i - does $i have a valid
                                   target */
  "istarget",                   /* if istarget $n - is $n mob's target */
  "exists",                     /* if exists $n - does $n exist somewhere */

  "affected",                   /* if affected $n blind - is $n affected by
                                   blind */
  ...
  "wears",                      /* if wears $n lantern - is $n wearing a
                                   'lantern' */
  /* if wears $n 1233 - is $n wearing obj vnum 1233 */
  "has",                        /* if has $n weapon - does $n have obj of type
                                   weapon */
  "uses",                       /* if uses $n armor - is $n wearing obj of type 
                                   armor */
  ...
  "money",                      /* if money $n */
  "objval0",                    /* if objval0 > 1000 - object value[] checks
                                   0..4 */
  "objval1",
  "objval2",
  "objval3",
             ...
             CHAR_DATA * rch)
{
  ...
  if (rval >= 0)
  {
    ...
    {
      sprintf(buf, "Cmd_eval: prog %ld syntax error(2) '%s'", vnum, original);
      bug(buf, 0);
      return false;
    }
  ...
  if (buf[0] != '$' || buf[1] == '\0')
  {
    sprintf(buf, "Cmd_eval: prog %ld syntax error(3) '%s'", vnum, original);
    bug(buf, 0);
    return false;
  }
  else
  ...
  switch (code)
  {
    ...
    case 'q':
      ...
      break;
    default:
      sprintf(buf, "Cmd_eval: prog %ld syntax error(4) '%s'", vnum, original);
      bug(buf, 0);
      return false;
  }
  /* 
  ...
  if ((oper = keyword_lookup(fn_evals, buf)) < 0)
  {
    sprintf(buf, "Cmd_eval: prog %ld syntax error(5): '%s'", vnum, original);
    bug(buf, 0);
    return false;
  }
  one_argument(line, buf);
                ...
                CHAR_DATA * rch)
{
  ...
  while (*str != '\0')
  {
    ...
    switch (*str)
    {
      ...
      case 'E':
        i = (vch != NULL &&
             can_see(mob, vch)) ? he_she[URANGE(0, vch->sex, 2)] : someone;
        break;
      case 'J':
        i = (rch != NULL &&
             can_see(mob, rch)) ? he_she[URANGE(0, rch->sex, 2)] : someone;
        break;
      case 'X':
        i = (mob->mprog_target != NULL &&
      ...
      case 'm':
        i = (ch != NULL &&
             can_see(mob, ch)) ? him_her[URANGE(0, ch->sex, 2)] : someone;
        break;
      case 'M':
        i = (vch != NULL &&
             can_see(mob, vch)) ? him_her[URANGE(0, vch->sex, 2)] : someone;
        break;
      case 'K':
        if (rch == NULL)
          rch = get_random_char(mob);
        i = (rch != NULL &&
             can_see(mob, rch)) ? him_her[URANGE(0, rch->sex, 2)] : someone;
        break;
      case 'Y':
        i = (mob->mprog_target != NULL &&
      ...
      case 's':
        i = (ch != NULL &&
             can_see(mob, ch)) ? his_her[URANGE(0, ch->sex, 2)] : someones;
        break;
      case 'S':
        i = (vch != NULL &&
             can_see(mob, vch)) ? his_her[URANGE(0, vch->sex, 2)] : someones;
        break;
      case 'L':
        if (rch == NULL)
          rch = get_random_char(mob);
        i = (rch != NULL &&
             can_see(mob, rch)) ? his_her[URANGE(0, rch->sex, 2)] : someones;
        break;
      case 'Z':
        i = (mob->mprog_target != NULL &&
                  ...
                  const void *arg2)
{
  ...
  {
    bug("MOBprogs: MAX_CALL_LEVEL exceeded, vnum %ld", mob->pIndexData->vnum);
    return;
  }
