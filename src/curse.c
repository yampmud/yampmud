...
CH_CMD(do_bane)
{
  ...
  if (arg[0] == '\0')
  {
    ...
    act("$N says '{aI offer the following services:{x'", ch, NULL, mob,
        TO_CHAR);
    send_to_char("  align: Darkness of the Gods       20 gold\n\r", ch);
    send_to_char("  sanctuary: The Gods' Protection   35 gold\n\r", ch);
  ...
  {
    act("$N says '{aType 'bane' for a list of spells.{x'", ch, NULL, mob,
        TO_CHAR);
    return;
  }
  ...
  {
    act("$N says '{aYour soul is not worthy of such blessings!{x'", ch,
        NULL, mob, TO_CHAR);
    return;
  }
  ...
  if (spell == NULL)            /* Decrease alignment toward evil */
  {
    ...
    send_to_char("You feel darkness enshroud your soul.\n\r", ch);
    return;
  }
