...
CH_CMD ( do_icn )
{
    ...
    parse_note ( ch, argument, NOTE_ICN );
}
...
void load_thread ( char *name, NOTE_DATA ** list, int type, time_t free_time )
{
    ...
    for ( ;; )
    {
        ...
        if ( str_cmp ( fread_word ( fp ), "tcolor" ) )
            break;
        pnote->tcolor = fread_number ( fp );
...
void update_read ( CHAR_DATA * ch, NOTE_DATA * pnote )
{
    ...
    switch ( pnote->type )
    {
        ...
        case NOTE_NOTE:
            ...
            break;
        case NOTE_ICN:
            ch->pcdata->last_icn = UMAX ( ch->pcdata->last_icn, stamp );
            break;
        case NOTE_PENALTY:
        ...
        case NOTE_WEDDINGS:
            ch->pcdata->last_weddings =
                UMAX ( ch->pcdata->last_weddings, stamp );
            break;
    }
}
...
/*                if ( pnote->tcolor > 0 )
                {
                    sprintf ( temp, " " );
                    strcpy ( temp, pnote->text );
                    ttcolor = pnote->tcolor;
                    pnote->text = pcolor ( NULL, temp, ttcolor );
                } */
                send_to_char ( buf, ch );
                page_to_char ( pnote->text, ch );
                update_read ( ch, pnote );
        ...
        if ( !vnum )
        {
            switch ( type )
            {
                case NOTE_NOTE:
                    ...
                    break;
                case NOTE_ICN:
                    sprintf ( buf, "There are no ICNs for you.\n\r" );
                    break;
                case NOTE_PENALTY:
    ...
    if ( !str_prefix ( arg, "catchup" ) )
    {
        switch ( type )
        {
            case NOTE_NOTE:
                ...
                break;
            case NOTE_ICN:
                ch->pcdata->last_icn = current_time;
                break;
            case NOTE_PENALTY:
    ...
    if ( !str_cmp ( arg, "+" ) )
    {
        ...
        {
            send_to_char ( "Sorry, notes have a 4k limit.\n\r", ch );
            return;
        }
        ...
        {
            sprintf ( buf,
                      "Please keep lines within 250 chars of length.\n\rYour line had %d chars.\n\rPlease start a new line.\n\r",
                      strlen ( argument ) );
            send_to_char ( buf, ch );
            return;
        }
    ...
    {
        send_to_char
            ( "This command has been disabled due to bugs.\n\rPlease see HELP NOTE.\n\rYou MAY use the note + and note - commands.\n\r",
              ch );
        return;
    }
...
/*    if ( !str_prefix ( arg, "tcolor" ) )
    ...
    if ( !str_prefix ( arg, "post" ) || !str_prefix ( arg, "send" ) )
    {
        ...
        sprintf ( buf,
                  "{WCheck your %s! {x[{cSender: {D%s{c Subject: {D%s{x]\n\r",
                  list_name, ch->name, ch->pnote->subject );
        ...
        for ( d = descriptor_list; d; d = d->next )
        {
            ...
            {
                if ( !str_cmp ( "all", ch->pnote->to_list ) )
