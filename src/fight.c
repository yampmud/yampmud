...
void multi_hit ( CHAR_DATA * ch, CHAR_DATA * victim, int dt )
{
    ...
    if ( ch->stunned )
    {
            ...
            ( ch->stunned ) ) * 2 ) )
        {
         ...
         if ( !ch->stunned )
         {
            ...
            act ( "$n regains $m equilibrium.", ch, NULL, NULL, TO_ROOM );
            return;
         } else { send_to_char ( "{RYou are stunned.{x\n\r", ch ); }
         return;
     }
    ...
    if ( IS_SHIELDED ( ch, SHD_LASTRITES ) && ( ch->hit <= ch->max_hit * .16 ) )
    {
        ...
        else send_to_char ( "You fail to recite your last rites.\n\r", ch );
    }
    ...
    if ( result && wield != NULL )
    {
        ...
        if ( ch->fighting == victim && IS_WEAPON_STAT ( wield, WEAPON_FROST ) )
        {
            ...
            xact_new ( "{iThe {Ccold{i touch of $p surrounds you with {Cice.{x",
                       victim, wield, NULL, TO_CHAR, POS_RESTING,
                       VERBOSE_FLAGS );
              ...
              bool show, int verbose )
{
    ...
    if ( victim->position == POS_DEAD || !WR ( ch, victim ) ||
         !WR ( victim, ch ) )
        return 0;
...
/*    if ( (dam > 1) && (dt>0) && (dt < TYPE_HIT ) &&
    ...
    if ( check_critical ( ch, victim ) && dam > 400 )
    {
        ...
        cchan = ( ( get_skill ( ch, gsn_critical ) / 4 ) + ( ( ch->level ) / 8 ) + ( number_range ( 1, 50 ) ) );
    ...
    if ( IS_SHIELDED ( victim, SHD_LIFE ) && ( dam > 0 ) && victim->mana > 0 
         && victim->max_mana > 0 && ( ( ( victim->mana * 100 ) / victim->max_mana ) > 20 ) 
         && !str_cmp ( class_table[victim->class].name, "Elder" ) && victim->level >= 202 )
    {
        char buf[MAX_STRING_LENGTH];
        ...
        send_to_char ( buf, victim );
        sprintf ( buf,
                  "$n's lifeforce absorbs the damage!{x" );
        act ( buf, victim, NULL, NULL, TO_ROOM );
    ...
    if ( victim->position == POS_DEAD )
    {
        ...
        if ( !IS_NPC ( ch ) && !IS_NPC ( victim ) && ch->name != victim->name )
        {
            ...
            if ( !WR ( ch, victim ) && !IS_IMMORTAL ( ch ) &&
                 !IS_IMMORTAL ( victim ) )
            {
                sprintf ( buf,
                   ...
                   bool immune, int verbose, bool singular )
{
    ...
    {
        vs = "{D..{w--{r=={R**{W( {Y!! {OI {ON {OS {OT {OA {ON {OT {OL {OY   {OK {OI {OL {OL {OS {Y!! {W){R**{r=={w--{D..{x";
        vp = "{D..{w--{r=={R**{W( {Y!! {OI {ON {OS {OT {OA {ON {OT {OL {OY   {OK {OI {OL {OL {OS {Y!! {W){R**{r=={w--{D..{x";
    }
...
*/
    ...
    else
    {
        ...
        if ( !IS_NPC ( ch ) )
            if ( !IS_SET ( ch->pcdata->verbose, VERBOSE_DAMAGE ) )
                xact_new ( buf2, ch, NULL, victim, TO_CHAR, POS_RESTING,
                           verbose );
        if ( !IS_NPC ( victim ) )
            if ( !IS_SET ( victim->pcdata->verbose, VERBOSE_DAMAGE ) )
                xact_new ( buf3, ch, NULL, victim, TO_VICT, POS_RESTING,
                           verbose );
    }
    ...
    if ( number_percent (  ) < chance )
    {
        ...
        if ( schance < chance )
        {
            ...
            { 
                ...
                act( "$N is virtually knocked out by your bash!", ch, NULL, victim, TO_CHAR ); 
                act( "$N has been virtually knocked out cold!", ch, NULL, victim, TO_NOTVICT); 
             } 
             else if ( schance < 10 ) 
             { 
                victim->stunned = 3; 
                act( "You have extremely stunned by a powerful bash!", ch, NULL, victim, TO_VICT ); 
                act( "$N has been extremely stunned by your bash!", ch, NULL, victim, TO_CHAR ); 
                act( "$N is reeling with glazed eyes from a powerful bash!", ch, NULL, victim, TO_NOTVICT ); 
             } 
             else
             {
                victim->stunned = 2;
                ...
                act( "$N looks a little stunned by the bash.", ch, NULL, victim, TO_NOTVICT ); 
             }
...
bool check_counter ( CHAR_DATA * ch, CHAR_DATA * victim, int dam, int dt )
{
    ...
    damage ( victim, ch, dam / 2, gsn_counter, dam_type, true );    /* DAM MSG
                                                                       NUMBER!! 
                                                                     */
         ...
         ( get_skill ( ch, gsn_feed ) >= 2 && !IS_AWAKE ( victim ) ) )
    {
        ...
        dam =
            number_range ( ( ( ch->level / 2 ) + ( victim->level / 2 ) * 40 ),
                           ( ( ch->level / 2 ) +
                             ( victim->level / 2 ) ) * 175 );
            damage ( ch, victim, dam, gsn_feed, DAM_PIERCE, true );
    }
    else
    {
...
CH_CMD ( do_howl )
{
    ...
    {
        printf_to_char ( ch, "You try to howl, but only produce a faint screech.\n\r" );
    ...
    dam = 35 + ch->level;
    damage ( ch, victim, number_range ( dam, ( ch->level * 4 ) ), gsn_howl,
             DAM_SOUND, true );
    check_improve ( ch, gsn_howl, true, 1 );

    if ( chance ( 55 ) )
        dam = 30 + ch->level;
    damage ( ch, victim, number_range ( dam, ( ch->level * 3 ) ), gsn_howl,
             DAM_SOUND, true );
...
CH_CMD ( do_wspit )
{
    ...
    dam = 35 + ch->level;
    damage ( ch, victim, number_range ( dam, ( ch->level * 4 ) ), gsn_wspit,
             DAM_WATER, true );
    check_improve ( ch, gsn_wspit, true, 1 );

    if ( chance ( 55 ) )
        dam = 30 + ch->level;
    damage ( ch, victim, number_range ( dam, ( ch->level * 3 ) ), gsn_wspit,
             DAM_WATER, true );
...
CH_CMD ( do_rub )
{
    ...
    if ( number_percent (  ) < chance )
    {
        ...
        if ( IS_AFFECTED ( ch, gsn_gouge ) && ( !str_cmp ( class_table[ch->class].name, "Knight" ) ) )
        {
    ...
    if ( chance >= 45 )
    {
        send_to_char
            ( "You slice your finger while trying to sharpen your weapon.\n\r",
              ch );
        damage ( ch, ch, ch->hit / 10, gsn_sharpen, WEAR_NONE, DAM_SLASH );
        return;
    }
...
CH_CMD ( do_rampage )
{
    ...
    damage ( ch, victim, number_range ( dam, ch->level * 30 ), gsn_rampage,
             DAM_BASH, true );
    ...
    damage ( ch, victim, number_range ( dam, ch->level * 55 ), gsn_rampage,
             DAM_BASH, true );
...
/*    if ( !can_see ( ch, victim ) )
    ...
    damage ( ch, victim, number_range ( dam, ch->level * 46 ), gsn_nervestrike,
             DAM_BASH, true );
...
CH_CMD ( do_mend )
{
    ...
    if ( victim != ch )
    {
       ...
       {
          send_to_char ( "You may only mend those who are in a comfortable position.\n\r", ch );
          return;
       }
    }
