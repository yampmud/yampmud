...
MAGIC(spell_entangle)
{
  ...
  send_to_char("{gV{Gi{yn{ge{Gs{x grow up from the ground and bind you.",
               victim);
  act("{gV{Gi{yn{ge{Gs{x grow up from the ground and bind $n.", victim, NULL,
      NULL, TO_ROOM);
  return;
}
...
MAGIC(spell_bark_skin)
{
  ...
  if (is_affected(victim, sn))
  {
    if (victim == ch)
      ...
      act("$N's skin is already the texture of bark.", ch, NULL, victim,
          TO_CHAR);
    return;
  }
...
MAGIC(spell_soul_siphon)
{
  ...
  act("You {Dsiphon{x off some of $N's {ds{do{du{dl{x.", ch, NULL, victim,
      TO_CHAR);
  act("$n {Dsiphons{x off some of your {ds{do{du{dl{x.", ch, NULL, victim,
      TO_VICT);
...
MAGIC(spell_immortal_wrath)
{
  ...
  {
    act
      ("The {BImm{Wort{Bals {xrelease their {DW{RR{WA{RT{DH {xupon {c$N!!!{x",
       ch, NULL, victim, TO_ROOM);
    act
...
MAGIC(spell_powersurge)
{
  ...
  if (is_affected(ch, sn))
  {
    if (victim == ch)
      ...
      act("This spell can't be cast on others! ", ch, NULL, victim, TO_CHAR);
    return;
  }
...
MAGIC(spell_shadow_barrier)
{
  ...
  if (is_affected(victim, sn))
  {
    if (victim == ch)
      ...
      act("$N is already protected by shadows.", ch, NULL, victim, TO_CHAR);
    return;
  }
  af.where = TO_AFFECTS;
  af.type = sn;
...
MAGIC(spell_downpour)
{
  ...
  act("$n raises $s arms and prays for {cr{wa{Ci{cn{x.", ch, NULL, NULL,
      TO_ROOM);
  act("You raise your arms and pray for {cr{wa{Ci{cn{x.", ch, NULL, NULL,
      TO_CHAR);
...
MAGIC(spell_dust_storm)
{
  ...
  act("$n summons a {yd{Du{Ys{yt st{Yo{Dr{ym{x to attack $N!", ch, NULL,
      victim, TO_NOTVICT);
  act("You are caught in $n's {yd{Du{Ys{yt st{Yo{Dr{ym{x!!", ch, NULL, victim,
      TO_VICT);
...
MAGIC(spell_ego_whip)
{
  ...
  act("$n prods your mind, looking for weaknesses.", ch, NULL, victim,
      TO_VICT);
...
MAGIC(spell_death_chant)
{
  ...
  if (victim->in_room == ch->in_room)
  {
    act("$n chants dark words, eyes grimly focused on $N.", ch, NULL, victim,
        TO_NOTVICT);
    act("$n chants dark words, eyes grimly focused on you!", ch, NULL, victim,
        TO_VICT);
    ...
    {                           // If my understanding is correct, this should be %0.25
      ...
      {
        act
          ("$N's eyes roll back as $S lifeless body falls to the ground.\n\r",
           ch, NULL, victim, TO_NOTVICT);
        act
          ("You are pulled in by $n's death chant.  Everything goes dark...\n\r",
           ch, NULL, victim, TO_VICT);
        act("$N collapses to the ground, surrending to your death chant.", ch,
            NULL, victim, TO_CHAR);
        if (!IS_NPC(victim))
        {
          sprintf(buf,
                  "{w[{RPK{w] {R%s {xhas {Yfallen {xto {R%s's {Ddeath magic{x {wat {Y%s{w!{x",
...
MAGIC(spell_terror)
{
  ...
  if (ch != victim)
    act("$N is suddenly stricken with {Dterror{x!", ch, NULL, victim,
        TO_CHAR);
...
MAGIC(spell_pandemonium)
{
  ...
  act("$n performs a devastatingly {nc{=h{-a{po{dt{Oi{Ic{x song!", ch, NULL,
      NULL, TO_ROOM);
  send_to_char
    ("You perform a devastatingly {nc{=h{-a{po{dt{Oi{Ic{x song!\n\r", ch);
  ...
  for (ich = ch->in_room->people; ich; ich = ich_next)  // Changed how we look for new players
  {
    ...
    if (chance == 1)
    {
      act("Summoned {Wd{wa{Dgge{wr{Ws{x rain down upon $N!", ch, NULL, ich,
          TO_NOTVICT);
      act("Summoned {Wd{wa{Dgge{wr{Ws{x rain down upon $N!", ch, NULL, ich,
          TO_CHAR);
    ...
    if (number_range(1, 100) < mal) // ( consider lowering.. dosn't happen very often =/ )
    {
      ...
      else if ((chance == 2) && (!IS_AFFECTED(ich, AFF_WEAKEN)))
      {
        ...
        act("$N seems weakened by the pandemonium.", ch, NULL, ich,
            TO_NOTVICT);
      ...
      else if ((chance == 5) && (!IS_AFFECTED(ich, AFF_FAERIE_FIRE)))
      {
        ...
        act("$N is surrounded by a {Mpink{x aura.", ch, NULL, ich,
            TO_NOTVICT);
      ...
      {
        act("$n shrugs off the affects of the pandemonium.", ch, NULL, ich,
            TO_NOTVICT);
        act("$N shrugs off the affects of the pandemonium.", ch, NULL, ich,
            TO_CHAR);
        send_to_char("You shrug off the chaotic affects of the pandemonium.",
                     ich);
      }
    }
    continue;                   // Next!
  ...
  if (!found)                   // Nobody around..
    send_to_char
      ("You cease playing and hear the sound of crickets chirping..\n\r", ch);
...
MAGIC(spell_gust)
{
  ...
  {
    act("You are blown backwards by $n's {Wgust{x!", ch, NULL, victim,
        TO_VICT);
    act("$N is blown backwards by your {Wgust{x!", ch, NULL, victim, TO_CHAR);
    act("$N is blown backwards by $n's {Wgust{x!", ch, NULL, victim,
        TO_NOTVICT);
...
MAGIC(spell_nightmare)
{
  ...
  if (victim->in_room == ch->in_room)
  {
    act("$n fills your head with {Dhorrific visions{x!", ch, NULL, victim,
        TO_VICT);
...
MAGIC(spell_icestorm)
{
  ...
  act
    ("{WS{wh{War{wd{Ws {gof {-i{-c{-e r{-a{-i{-n {gcalled by $n shower down upon {R$N{g!{x",
     ch, NULL, victim, TO_NOTVICT);
  act
    ("{WS{wh{War{wd{Ws {gof {-i{-c{-e r{-a{-i{-n {gcalled by $n shower down upon {Ryou{g!{x",
     ch, NULL, victim, TO_VICT);
  act
  ...
  for (vch = char_list; vch != NULL; vch = vch_next)
  {
    ...
    if (vch->in_room == ch->in_room)
    {
      if (IS_EVIL(vch))
      {
        ...
        spell_frenzy(skill_lookup("frenzy"), level, ch, (void *) vch,
                     TARGET_CHAR);
        spell_protection_good(skill_lookup("protection good"), level, ch,
  ...
  for (vch = char_list; vch != NULL; vch = vch_next)
  {
    ...
    if (vch->in_room == ch->in_room)
    {
      if (IS_GOOD(vch))
      {
        ...
        spell_protection_evil(skill_lookup("protection evil"), level, ch,
                              (void *) vch, TARGET_CHAR);
        spell_bless(skill_lookup("bless"), level, ch, (void *) vch,
                    TARGET_CHAR);
      }
      else if (vch != ch && !is_safe_spell(ch, vch, true))
      {
...
MAGIC(spell_disrupt)
{
  ...
  chance =
    15 + (get_curr_stat(ch, STAT_INT) - get_curr_stat(victim, STAT_INT));
  ...
  if (number_percent() <= chance)
  {
    act("You are sent staggering by $n's {cr{ge{Gp{Bu{bl{Bs{Gi{go{cn{x!", ch,
        NULL, victim, TO_VICT);
    act("$N is sent staggering by your {cr{ge{Gp{Bu{bl{Bs{Gi{go{cn{x!", ch,
        NULL, victim, TO_CHAR);
    act("$N is sent staggering by $n's {cr{ge{Gp{Bu{bl{Bs{Gi{go{cn{x!", ch,
...
MAGIC(spell_repulse)
{
  ...
  chance =
    20 + (get_curr_stat(ch, STAT_INT) - get_curr_stat(victim, STAT_INT));
  ...
  if (number_percent() <= chance)
  {
    act("You are sent staggering by $n's {Dr{me{Mp{Bu{bl{Bs{Mi{mo{Dn{x!", ch,
        NULL, victim, TO_VICT);
    act("$N is sent staggering by your {Dr{me{Mp{Bu{bl{Bs{Mi{mo{Dn{x!", ch,
        NULL, victim, TO_CHAR);
    act("$N is sent staggering by $n's {Dr{me{Mp{Bu{bl{Bs{Mi{mo{Dn{x!", ch,
        NULL, victim, TO_NOTVICT);
...
MAGIC(spell_cyclone)
{
  ...
  if (number_percent() <= chance)
  {
    act("You are blown to the ground by $n's {Wcyclone{x!", ch, NULL, victim,
        TO_VICT);
    act("$N is blown to the ground by your {Wcyclone{x!", ch, NULL, victim,
        TO_CHAR);
    act("$N is blown to the ground by $n's {Wcyclone{x!", ch, NULL, victim,
        TO_NOTVICT);
    DAZE_STATE(victim, 2 * PULSE_VIOLENCE);
    victim->position = POS_RESTING;
...
MAGIC(spell_death_knell)
{
  ...
  act("$n draws the last of your life from your body...", ch, NULL, victim,
      TO_VICT);
  act("You drain the last of $N's life from $S body and grow stronger!", ch,
      NULL, victim, TO_CHAR);
...
MAGIC(spell_prismatic_spray)
{
  ...
  while (beams > 0)
  {
    ...
    if (color == 7)
    {
      ...
      act("$N is hit by a beam of {mviolet light{x and is sent away!", ch,
          NULL, victim, TO_CHAR);
      act("You are hit by a beam of {mviolet light{x and are sent away!", ch,
          NULL, victim, TO_VICT);
    ...
    if (color == 6)
    {
      ...
      act("$N is hit by a beam of {bindigo light{x!", ch, NULL, victim,
          TO_CHAR);
      act("You feel drained as you are hit by a beam of {bindigo light{x!",
          ch, NULL, victim, TO_VICT);
    ...
    if (color == 5)
    {
      ...
      act
        ("$N is hit by a beam of {Bblue light{x and $S skin begins to harden!",
         ch, NULL, victim, TO_CHAR);
      act
        ("Your skin begins to harden as you are hit by a beam of {Bblue light{x!",
         ch, NULL, victim, TO_VICT);
    ...
    if (color == 4)
    {
      ...
      act("$N is hit by a beam of {Ggreen light{x and looks very ill.", ch,
          NULL, victim, TO_CHAR);
      act("You feel sick as you are hit by a beam of {Ggreen light{x!", ch,
          NULL, victim, TO_VICT);
      if (saves_spell(level, victim, DAM_POISON))
        dam /= 2;
    ...
    if (color == 3)
    {
      ...
      act("$N is hit by a beam of {Yyellow light{x!", ch, NULL, victim,
          TO_CHAR);
      act("You are shocked by a beam of {Yyellow light{x!", ch, NULL, victim,
          TO_VICT);
    ...
    if (color == 2)
    {
      ...
      act("$N is hit by a beam of {yorange light{x!", ch, NULL, victim,
          TO_CHAR);
      act("You are hit with acid from a beam of {yorange light{x!", ch, NULL,
          victim, TO_VICT);
      if (saves_spell(level, victim, DAM_ACID))
        dam /= 2;
      damage_old(ch, victim, dam, sn, DAM_ACID, true);
    ...
    if (color == 1)
    {
      ...
      act("You are burned by a beam of {Rred light{x!", ch, NULL, victim,
          TO_VICT);
      if (saves_spell(level, victim, DAM_FIRE))
        dam /= 2;
...
MAGIC(spell_elysium)
{
  ...
  act("You attempt to send $N into the paradise beyond death!", ch, NULL,
      victim, TO_CHAR);
  act("$n attempts to send you into the paradise beyond death!", ch, NULL,
      victim, TO_VICT);
            ...
            number_range(1, 35) > get_curr_stat(vch, STAT_CON))
        {
          ...
          send_to_char
            ("You scream in agony as plague sores erupt from your skin.\n\r",
             vch);
          act("$n screams in agony as sores erupt all over $s skin.", vch,
              NULL, NULL, TO_ROOM);
        }
      }
      continue;
...
MAGIC(spell_baptism)
{
  ...
  act("$n dunks you in the water and baptizes you!", ch, NULL, victim,
      TO_VICT);
  act("$n dunks $N in the water and baptizes $M!", ch, NULL, victim,
      TO_NOTVICT);

  if ((IS_EVIL(ch) && IS_EVIL(victim)) ||
      (IS_NEUTRAL(ch) && IS_NEUTRAL(victim)) || (IS_GOOD(ch) &&
                                                 IS_GOOD(victim)))
  {
    spell_bless(skill_lookup("bless"), level, ch, (void *) victim,
                TARGET_CHAR);
...
MAGIC(spell_pollution)
{
  ...
  act
    ("You conjure a thick cloud of {Dpo{gl{Gl{Dut{Gi{go{yn{x to surround $N!",
     ch, NULL, victim, TO_CHAR);
  act
    ("$n conjures a thick cloud of {Dpo{gl{Gl{Dut{Gi{go{yn{x to surround you!",
     ch, NULL, victim, TO_VICT);
  act
    ("$n conjures a thick cloud of {Dpo{gl{Gl{Dut{Gi{go{yn{x to surround $N!",
     ch, NULL, victim, TO_NOTVICT);
...
MAGIC(spell_phantasmal_killer)
{
  ...
  act("You dig into $N's mind and summon $S worst nightmare!", ch, NULL,
      victim, TO_CHAR);
  act
    ("The most horrific being you could imagine appears before you and attacks!",
     ch, NULL, victim, TO_VICT);
...
MAGIC(spell_vermin_swarm)
{
  ...
  act("You summon a swarm of rats and insects to attack $N!", ch, NULL,
      victim, TO_CHAR);
  act("Rats and insects summoned by $n rapidly overtake you!", ch, NULL,
      victim, TO_VICT);
  act
    ("$N is viciously attacked by a swarm of rats and insects summoned by $N.",
     ch, NULL, victim, TO_NOTVICT);
...
MAGIC(spell_disintegrate)
{
  ...
  if (victim->in_room == ch->in_room)
  {
    act("$n shoots a {gd{Gi{gs{Gi{gntegrat{Gi{gon {Gb{gea{gm{x at $N!!!", ch,
        NULL, victim, TO_ROOM);
...
MAGIC(spell_diamond_aura)
{
  ...
  {
    send_to_char
      ("Your body is too accustomed to the heat to bar itself against the cold.\n\r",
       ch);
    return;
  }
  ...
  act("$n is surrounded by a glittering white aura.", ch, NULL, NULL,
      TO_ROOM);
  return;
}
...
MAGIC(spell_ruby_aura)
{
  ...
  {
    send_to_char
      ("Your body is too accustomed to the cold to bar itself against the heat.\n\r",
       ch);
    return;
  }
...
MAGIC(spell_topaz_aura)
{
  ...
  {
    send_to_char
      ("You are too aligned with earth to guard against lightning.\n\r", ch);
    return;
  }
...
MAGIC(spell_obsidian_aura)
{
  ...
  {
    send_to_char
      ("You are too aligned with lightning to guard against earth.\n\r", ch);
    return;
  }
