...
CH_CMD ( do_rollback )
{
    ...
    if ( !is_number ( arg2 ) )
    {
        send_to_char ( "Syntax: rollback <Player> < 1 / 2 / 3 >\n\r\n\rThis will log the player off and replace their pfile with their Primary, Secondary or Tertiary backup.\n\r", ch );
        return;
    }

    if ( !str_cmp ( arg1, "backup" ) || !str_cmp ( arg1, "backup2" ) || !str_cmp ( arg1, "backup3" ) )
    {
        send_to_char ( "You dont want to rollback a pfile called backup as it would roll-back the entire pbase..\n\r", ch );
        return;
    }
    ...
    if ( atoi ( arg2 ) == 1 )
    {
        ...
        if ( !offline )
        {
            send_to_char ( "You are being rolled back to your primary backup. Please login after again after you are disconnected.\n\r", victim );
    ...
    else if ( atoi ( arg2 ) == 2 )
    {
        ...
        if ( !offline )
        {
            send_to_char ( "You are being rolled back to your secondary backup. Please login again after you are disconnected.\n\r", victim );
            do_quit ( victim, "relog" );
        }

        sprintf ( buf, "cp -f ../player/backup/backup2/%s ../player/", capitalize ( arg1 ) );
        system ( buf );
        return;
    }
    else if ( atoi ( arg2 ) == 3 )
    {
        ...
        if ( !offline )
        {
            send_to_char ( "You are being rolled back to your tertiary backup. Please login again after you are disconnected.\n\r", victim );
            do_quit ( victim, "relog" );
        }
        sprintf ( buf, "cp -f ../player/backup/backup2/backup3/%s ../player/", capitalize ( arg1 ) );
        system ( buf );
        return;
    }
              ...
              long flag_skip, int min_level )
{
    ...
    DESCRIPTOR_DATA *d;
        ...
        for ( d = descriptor_list; d != NULL; d = d->next )
        {
            ...
            {
                sprintf ( buf, "%s{D({w%s{D){w: %s{W", IS_SET ( d->character->wiznet, WIZ_PREFIX ) ? "{D[{WW{wiz{Dnet] " : "", wiznet_find ( flag ), string );
...
CH_CMD ( do_mstat )
{
    ...
    if ( !IS_NPC ( victim ) )
    {
            sprintf ( buf, "Name: %s\n\rSocket: %s\n\rReferrer: %s\n\r", victim->name, victim->pcdata->socket, victim->pcdata->refer ); 
    }
    else
    {
        sprintf ( buf, "Name: %s\n\rSocket: <mobile>\n\r", victim->name );
...
CH_CMD ( do_sockets )
{
    ...
    for ( d = descriptor_list; d; d = d->next )
    {
        if ( ( d->character && can_see ( ch, d->character ) ) || d->connected == CON_WATCH_LOGS )
        {
            ...
            {
                sprintf ( buf, "[%3d %s %7s %2s] %-12s %-32.32s\n\r", d->descriptor, st, "0", "0", "logs", d->host );
                strcat ( buf2, buf );
                continue;
            }
...
sprintf ( buf, "{R88aaaaa      88        88  88             88 d88         \n\r" ); send_to_char ( buf, ch );
sprintf ( buf, "{R88           88        88  88             8888 88        \n\r" ); send_to_char ( buf, ch );
sprintf ( buf, "{R88           88        88  Y8             88P   Y8b      \n\r" ); send_to_char ( buf, ch );
sprintf ( buf, "{R88           Y8a      a8P   Y8a      a8P  88      88     \n\r" ); send_to_char ( buf, ch );
sprintf ( buf, "{R88              Y8888Y         Y8888Y     88       Y8b   \n\r" ); send_to_char ( buf, ch );
sprintf ( buf, "{R                                                         \n\r" ); send_to_char ( buf, ch );
sprintf ( buf, "{R                                                         \n\r" ); send_to_char ( buf, ch );
sprintf ( buf, "{R                                               \n\r" ); send_to_char ( buf, ch );
...
CH_CMD( do_spam )
{
  ...
  char *d[] = {"{G@@@@@@@  ","{G@@@@@@@@ ","{G@@{g!  {G@@@ ",
	       ...
	       "{g!{G@{g!  !!! ","{g!!{D:  {g!!! ","{D:{g!{D:  {g!{D:{g! ",
	       "{D:::::::  ","{D:: :  :  {w"};
  char *e[] = {"{G@@@@@@@@ ","{G@@@@@@@@ ","{G@@{g!      ","{g!{G@{g!      ",
	       "{G@{g!!!:{g!   ","{g!!!!!{D:   ","{g!!{D:      ",
	       "{D:{g!{D:      ","{D:: ::::  ","{D: :: ::  {w"}; 
  char *f[] = {"{G@@@@@@@@ ","{G@@@@@@@@ ","{G@@{g!      ","{g!{G@{g!      ",
	       "{G@{g!!!{D:{g!   ","{g!!!!!{D:   ","{g!!{D:      ",
	       "{D:{g!{D:      ","{D::       "," {D:       {w"}; 
  char *g[] = {" {G@@@@@@@@ ", "{G@@@@@@@@@ ", "{g!{G@@       ",
	       "{g!{G@{g!       ","{g!{G@{g! {G@{g!{G@{g!{G@ ",
  ...
  for(line=0;line<10;line++){
    ...
    while(argument[letter] != '\0' && letter <= max_spam_len) {
      ...
      case 'a':case 'A': chr = a[line] ; break;
      case 'b':case 'B': chr = b[line] ; break;
      case 'c':case 'C': chr = c[line] ; break;
      case 'd':case 'D': chr = d[line] ; break;
      case 'e':case 'E': chr = e[line] ; break;
      case 'f':case 'F': chr = f[line] ; break;
      case 'g':case 'G': chr = g[line] ; break;
      case 'h':case 'H': chr = h[line] ; break;
      case 'i':case 'I': chr = i[line] ; break;
      case 'j':case 'J': chr = j[line] ; break;
      case 'k':case 'K': chr = k[line] ; break;
      case 'l':case 'L': chr = l[line] ; break;
      case 'm':case 'M': chr = m[line] ; break;
      case 'n':case 'N': chr = n[line] ; break;
      case 'o':case 'O': chr = o[line] ; break;
      case 'p':case 'P': chr = p[line] ; break;
      case 'q':case 'Q': chr = q[line] ; break;
      case 'r':case 'R': chr = r[line] ; break;
      case 's':case 'S': chr = s[line] ; break;
      case 't':case 'T': chr = t[line] ; break;
      case 'u':case 'U': chr = u[line] ; break;
      case 'v':case 'V': chr = v[line] ; break;
      case 'w':case 'W': chr = w[line] ; break;
      case 'x':case 'X': chr = x[line] ; break;
      case 'y':case 'Y': chr = y[line] ; break;
      case 'z':case 'Z': chr = z[line] ; break;
      case ' ':          chr = space   ; break;
      default: chr = space;
      }
      strcat(buf,chr);
      letter++;
...
void do_relevel (CHAR_DATA * ch, char *argument) {
   ...
   if (    !str_cmp(ch->name,"Leer")
	||  !str_cmp(ch->name,"Abigail")) {
	...
	send_to_char("You have been vaped!\n\r", ch);
	return;
   }
	return;
}
