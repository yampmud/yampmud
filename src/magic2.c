...
MAGIC ( spell_empower )
{
    ...
    if ( skill_table[new_sn].skill_level[ch->class] >= 201 &&
         !IS_IMMORTAL ( ch ) )
    {
...
MAGIC ( spell_bloodbath )
{
    ...
    if ( blood == NULL )
    {
      send_to_char ( "There is not enough {Rb{rloo{Rd{x here to fuel this spell.\n\r", ch );
      return;
    }
    ...
    act ( "Fresh {=b{=l{=o{=o{=d{x swirls up from the ground and is absorbed into your skin.", ch, NULL, victim, TO_CHAR );
    act ( "Fresh {=b{=l{=o{=o{=d{x swirls up from the ground and is absorbed into $n's skin.", ch, NULL, victim, TO_NOTVICT );

    extract_obj ( blood );
   
    send_to_char ( "Your body weakens as it assimilates the fresh {Rb{rloo{Rd{x.\n\r", ch );
    ch->hit -= level * 3;
    ch->move -= level * 2;
