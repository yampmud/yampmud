...
const struct quest_type quest_table[] = {
    ...
    {"crest", "{DC{Cres{Dt{x of the {YA{yn{Yc{yi{Ye{yn{Yt{ys{x", QUEST_CREST,
     5000},
    {"backpack", "{GB{yackpac{Gk{x of the {YA{yn{Yc{yi{Ye{yn{Yt{ys{x", QUEST_BACKPACK,
     5000},
    {"staff", "{yS{Dt{ya{Df{yf{x of the {YA{yn{Yc{yi{Ye{yn{Yt{ys{x", QUEST_STAFF,
     4000},
...
//        printf_to_char(ch,"        500  qp ........ {R25 {WI{wqp ({Wquest buy iqp{x){x\n\r");  
        ...
        send_to_char
            ( "\tFor more info on quest items type 'help qitems'\n\r", ch );
        return;
    }
    ...
    else if ( !str_cmp ( arg1, "sell" ) )
    {
        ...
        for ( i = 0; quest_table[i].name != NULL; i++ )
        {
            ...
            if ( quest_table[i].vnum == obj->pIndexData->vnum )
            {
                ...
                sprintf ( buf, "$N takes $p from you for %d quest points.", (int) ( quest_table[i].cost / 1.5 ) );
    ...
    else if ( !str_cmp ( arg1, "complete" ) )
    {
        ...
        if ( IS_SET ( ch->act, PLR_QUESTOR ) )
        {
            ...
            if ( ch->pcdata->questmob == -1 && ch->pcdata->countdown > 0 )
            {
                ...
                points =
                    ( number_range ( 25, 50 ) * ( ( ch->btime / 40 ) + 1 ) );
        ...
        switch ( number_range ( 0, 3 ) )
        {
            ...
            case 0:
                sprintf ( buf,
                          ...
                          victim->short_descr );
                do_mob_tell ( ch, questman, buf );
                do_mob_tell ( ch, questman, "King Garm has authorized an award to the one who eliminates this threat." );
                break;
            case 1:
                sprintf ( buf,
