        ...
        switch (pReset->command)
        {
          case 'M':
            if (!(pMob = get_mob_index(pReset->arg1)))
              bug("Convert_objects: 'M': bad vnum %ld.", pReset->arg1);
            break;

          case 'O':
            if (!(pObj = get_obj_index(pReset->arg1)))
            {
              bug("Convert_objects: 'O': bad vnum %ld.", pReset->arg1);
              break;
            }
            ...
            pObj->level =
              pObj->level <
              1 ? pMob->level - 2 : UMIN(pObj->level, pMob->level - 2);
            break;
          ...
          case 'P':
            {
              ...
              if (!(pObj = get_obj_index(pReset->arg1)))
              {
                bug("Convert_objects: 'P': bad vnum %ld.", pReset->arg1);
                break;
              }
              ...
              {
                bug("Convert_objects: 'P': bad vnum %ld.", pReset->arg3);
                break;
              }

              pObj->level =
                pObj->level <
                1 ? pObjTo->level : UMIN(pObj->level, pObjTo->level);
            }
            break;
          ...
          case 'E':
            ...
            if (!pMob)
            {
              bug("Convert_objects: 'E' or 'G': null mob for vnum %ld.",
                  pReset->arg1);
              break;
            }
