...
/*	 1,  2,  3,  4,  5, 25, 26,
	...
	15, 16, 19, 17, 18, 24,
	0*/
    6, 25, 26, 20, 3, 4, 10, 14, 15, 9, 1, 2, 16, 19, 11, 17, 0, 5, 28, 12, 13, 7,
    21, 22, 8, 18, 24, 23, 27
};
...
CH_CMD ( do_prompt )
{
    ...
    else if ( !str_cmp ( argument, "imm" ) )
    {
        ...
        {
            strcpy ( buf,
                     "{r%h-{R%Hhp {b%m-{B%Mm {g%v-{G%Vmv {C%Xtl {Y%bx{w>{x" );
        }
    }
...
CH_CMD ( do_lscore )
{
    ...
    printf_to_char ( ch,
                     ...
                     ch->move, ch->max_move, ch->carry_number,
                     can_carry_n ( ch ) );
    printf_to_char ( ch,
    ...
    {                           /* HERO's don't level anymore */
        ...
        printf_to_char ( ch,
                         ...
                         ch->gold, ch->platinum, ch->pcdata->balance,
                         ch->silver );
        if ( ch->pcdata->shares )
        {
            printf_to_char ( ch,
...
CH_CMD ( do_time )
{
    ...
    send_to_char ( buf, ch );
    sprintf ( buf,
              "{cDistortions of Chaos started up at {W%s{x\n\r{cThe system time is {W%s{x\n\r",
        ...
        if ( !str_prefix ( arg, wch->name ) )
        {
            ...
            switch ( wch->level )
            {
                ...
                case MAX_LEVEL - 3:
                    sprintf ( buf2, "{GDEI{x" );
                    break;
                    case MAX_LEVEL - 4:sprintf ( buf2, "{GGOD{x" );
...
/*    sprintf ( buf,
    ...
    sprintf ( buf,  " {D| {wSex Lvl Race Class {D[  {wFlags  {D] [    {wClan    {D] {wName   Title {D|  {CSee help who {D|\n\r" );
    send_to_char ( buf, ch ); 
    send_to_char ( sepbuf, ch ); 
...
//                        continue;
                    ...
                    if ( wch->pk_timer > 0 && is_pkill ( wch ) &&
                         !IS_IMMORTAL ( wch ) )
                        sprintf ( pkbuf, "{YT" );
                    ...
                    sprintf ( buf,
                              ...
                              "[    ???     ] " : clandat, !can_see ( ch,
                                                                      wch ) ?
                              pcolor ( wch, "Someone", 0 ) : pcolor ( wch, wch->name, 0 ), !can_see (ch, wch ) ? " " : wch->pcdata->title );
    ...
    {
        sprintf ( buf2, " {D| {wTotal People Connected: {C%-2d {D| {wMost On Today: {C%-2d   {D| {wMost On Ever: {C%-2d        {D|\n\r {D| {wLogins This Copyover:  {C%-2ld  {D| {wLogin This Hour: {C%-2ld "
        ...
        sprintf ( boardbuf, " {D| {wThe DoC Discussion Board is located at:      {Chttp://bhmm.net/forum/doc/    {D|\n\r" );
...
CH_CMD ( do_practice )
{
    ...
    if ( argument[0] == '\0' )
    {
        ...
        for ( sn = 2; sn < MAX_SKILL; sn++ )
        {
            ...
            send_to_char ( buf, ch );
            sprintf ( buf, "%19s %s%%%-3d {w|", skill_table[sn].name, ch->pcdata->learned[sn] < 26 ? "{r" : ch->pcdata->learned[sn] < 51 ? "{y" : ch->pcdata->learned[sn] < 76 ? "{g" : ch->pcdata->learned[sn] < 100 ? "{c" : ch->pcdata->learned[sn] >= 100 ? "{W" : "{W", ch->pcdata->learned[sn] );
            send_to_char ( buf, ch );
            if ( ++col > 2 )
            {
                sprintf ( buf, "\n\r" );
...
CH_CMD ( do_omni )
{
    ...
    for ( d = descriptor_list; d != NULL; d = d->next )
    {
        ...
        sprintf ( buf,
                  ...
                  wch->level, hptemp, wch->in_room->vnum, wch->btime,
                  wch->bflip );
        add_buf ( output, buf );
    }
