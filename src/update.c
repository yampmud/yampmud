...
void weather_update ( void )
{
    ...
    switch ( ++time_info.hour )
    {
        ...
        case 19:
            ...
            strcat ( buf,
                     "{yThe {Ysun{y slowly disappears in the west.{x\n\r" );
            break;
        case 20:
    ...
    switch ( weather_info.sky )
    {
        ...
        case SKY_LIGHTNING:
            ...
            {
                strcat ( buf,
                         "{DThe {xl{Ci{xg{Ch{xt{Cn{xi{Cn{xg has stopped.{x\n\r" );
                weather_info.sky = SKY_RAINING;
                break;
            }
            break;
...
/*    if ( IS_LIVEMUD )
    ...
    if ( arena == FIGHT_OPEN )
    {
        ...
        if ( mass_arena_valid_ticks >= 1 && !is_mass_arena &&
             !is_mass_arena_fight )
        {
            do_gmessage
    ...
    for ( ch = char_list; ch != NULL; ch = ch_next )
    {
        ...
        if ( !IS_NPC ( ch ) )
        {
                 ...
                 is_name ( ch->name, lastwinner ) )
            {
                ...
                sprintf ( lastwinner, "Nobody" );
                send_to_char
                    ( "You have been awarded 10iqp for your victory!\n\r",
                      ch );
                REMOVE_BIT ( ch->act2, PLR2_MASS_JOINER );
        ...
        if ( ( ( ( ch->played + ( int ) ( current_time - ch->logon ) ) / 3600 ) == 10 ) && str_cmp ( ch->pcdata->refer, "None" ) && !IS_SET ( ch->act2, PLR2_REFER ) && ch->pcdata->refer != NULL )
        {
            sprintf ( rewbuf, "System Message: You are now 10 hours old. If the player ( %s ) on this mud refered you please notify an immortal so the player can receive their referrer reward. Otherwise, ignore this message.\n\r", ch->pcdata->refer );
        ...
        {
            sprintf ( tbuf, "You have {R%d{x tells waiting.\n\rType 'replay' to see tells.\n\r", ch->tells );
        ...
        {       
            ...
            send_to_char ( "Something has gone wrong with your pfile!\007\n\rDisconnecting...\n\r", ch );
            break;
        }
