...
void read_from_buffer(DESCRIPTOR_DATA * d)
{
  ...
  if (k > 1 || d->incomm[0] == '!')
  {
    ...
    else
    {
      ...
      {
        sprintf(log_buf, "%s is spamming '%s'!", d->character->name,
                d->incomm);
      ...
      else if (d->repeat == 35 && d->character && d->connected == CON_PLAYING)
      {
        sprintf(log_buf, "%s is STILL spamming '%s'!", d->character->name,
                d->incomm);
        wiznet(log_buf, d->character, NULL, WIZ_SPAM, 0, 0);
        log_string(log_buf);
      ...
      else if (d->repeat >= 45 && d->character && d->connected == CON_PLAYING)
      {
        d->repeat = 0;

        sprintf(log_buf, "%s KICKED for spamming '%s'!", d->character->name,
                d->incomm);
        wiznet(log_buf, d->character, NULL, WIZ_SPAM, 0, 0);
        log_string(log_buf);
  ...
  switch (d->connected)
  {
    ...
    case CON_GET_NAME:
      ...
      else
      {
        ...
        sprintf(buf, "Are you sure you want to be called %s? (Y/N)? ",
                argument);
        write_to_buffer(d, buf, 0);
    ...
    case CON_GET_OLD_PASSWORD:
      ...
      for (pos = 0; pos < MAX_DUPES; pos++)
      {
        ...
        if ((victim = get_char_world(ch, ch->pcdata->dupes[pos])) != NULL)
          force_quit(victim, "");
      }
    ...
    case CON_GET_NEW_RACE:
          ...
          !IS_SET(ch->pcdata->ctier, pc_race_table[race].tier))
      {
        ...
        write_to_buffer(d, "{RThe following races are available:\n\r{x  ", 0);
        write_to_buffer(d,
    ...
    case CON_DEFAULT_CHOICE:
      ...
      switch (argument[0])
      {
        ...
        case 'N':
          ...
          break;
        default:
          write_to_buffer(d, "{WPlease answer {w({YY{w/{RN{w){W?{x ", 0);
          return;
      }
      break;
...
bool check_parse_name(char *name)
{
  ...
  if (!str_infix("dist", name) && str_cmp("Distortions", name))
    return false;
  if (!str_infix("shar", name) && str_cmp("Shartyn", name))
    return false;
  if (!str_infix("haos", name) && str_cmp("Khaos", name))
    return false;
  if (!str_infix("sam", name) && str_cmp("Samoth", name))
    return false;
...
void copyover_recover()
{
  ...
  for (;;)
  {
    ...
    else                        /* ok! */
    {
      ...
      write_to_descriptor(desc,
                          "You have been restored for the inconvenience.\n\r",
                          0);
      /*            if ( d->character->pet )
         {
