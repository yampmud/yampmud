...
CH_CMD ( do_drop )
{
    ...
    if ( str_cmp ( arg, "all" ) && str_prefix ( "all.", arg ) )
    {
        ...
        if ( IS_OBJ_STAT ( obj, ITEM_MELT_DROP ) )
        {
            act ( "You drop $p and it dissolves into smoke.", ch, obj, NULL,
                  TO_CHAR );
            act ( "$n drops $p and it dissolves into smoke.", ch, obj, NULL,
                  TO_ROOM );
            extract_obj ( obj );
        }
    }
...
void wear_obj ( CHAR_DATA * ch, OBJ_DATA * obj, bool fReplace, bool silent )
{
    ...
    if ( CAN_WEAR ( obj, ITEM_WEAR_FLOAT ) )
    {
        if ( !remove_obj ( ch, WEAR_FLOAT, fReplace, false ) )
            return;
        if ( !silent )
            act ( "$n releases $p to float next to $m.", ch, obj, NULL,
                  TO_ROOM );
        if ( !silent )
    ...
    if ( CAN_WEAR ( obj, ITEM_WEAR_ANKLE ) )
    {
        ...
        if ( get_eq_char ( ch, WEAR_ANKLE_L ) == NULL )
        {
            ...
            if ( !silent )
                act ( "You wear $p on your left ankle.", ch, obj, NULL,
                      TO_CHAR );
        ...
        {
            if ( !silent )
                act ( "$n wears $p on $s right ankle.", ch, obj, NULL,
                      TO_ROOM );
            if ( !silent )
    ...
    if ( CAN_WEAR ( obj, ITEM_WEAR_EAR ) )
    {
        ...
        if ( get_eq_char ( ch, WEAR_EAR_R ) == NULL )
        {
            ...
            if ( !silent )
                act ( "You wear $p on your right ear.", ch, obj, NULL,
                      TO_CHAR );
    ...
    if ( !str_cmp ( "all", arg ) || !str_prefix ( "all.", arg ) )
    {
        ...
        for ( obj = ch->in_room->contents; obj; obj = obj_next )
        {
                 ...
                 obj->item_type == ITEM_CORPSE_NPC )
            {
                ...
                act ( "$p suddenly flys upwards into the sky!", ch, obj, NULL,
                      TO_ROOM );
                act ( "You sacrifice $p to the altar of flesh and it flys upwards into the sky.", ch, obj, NULL, TO_CHAR );
            }
            else
            {
                extract_obj ( obj );
...
CH_CMD ( do_perform )
{
    ...
    if ( instrument->value[2] > 0 )
    {
        ...
        {
            act ( "Your performance with $p is unimpressive.", ch, instrument, NULL, TO_CHAR );
            act ( "...and nothing happens.", ch, NULL, NULL, TO_ROOM );
    ...
    if ( --instrument->value[2] <= 0 )
    {
        act ( "$p falls apart as $n plays $s last note.", ch, instrument, NULL, TO_ROOM );
         ...
         || ( !IS_NPC ( ch ) && !is_clan ( ch ) ) )
    {
        ...
        switch ( number_range ( 0, 3 ) )
        {
            ...
            case 2:
                ...
                break;
            case 3:
                sprintf ( buf, "{RKeep your hands out of there, %s{x{R!{x",
                          ch->name );
                break;
        }
        do_yell ( victim, buf );
