...
MAGIC ( spell_change_sex )
{
    ...
    if ( victim-> sex == 0 ) { change = ( number_range ( 1, 2 ) ); }
    else if ( victim->sex == 2) { change = 0 - ( number_range ( 1, 2 ) ); }
    else if ( number_range ( 1, 20 ) <= 10 ) { change = -1; }
    else { change = 1; }
...
MAGIC ( spell_chill_touch )
{
    ...
    if ( is_affected ( ch, sn ) )
    {
        if ( victim == ch )
            ...
            act ( "$N's skins already as cold as it can be.", ch, NULL, victim, TO_CHAR );
        return;
    }
...
MAGIC ( spell_create_rose )
{
    ...
    if ( ch->alignment > 400 )
    {
        ...
        act ( "$n has created a beautiful {Wwhite rose{x.", ch, rose, NULL, TO_ROOM );
    ...
    if ( ch->alignment < -400 )
    {
        ...
        act ( "$n has created a beautiful {Dblack rose{x.", ch, rose, NULL, TO_ROOM );
...
MAGIC ( spell_angeldust )
{
    ...
    if ( victim != ch )
    {
        act ( "$n calls forth heavenly angels to fight $N!", ch, NULL, victim, TO_ROOM );
        act ( "$n calls forth heavenly angels to attack you!", ch, NULL, victim, TO_VICT );
...
MAGIC ( spell_divinewrath )
{
    ...
    if ( number_range ( 1, 100 ) >= 98 )
    {
        ...
        act ( "$n is {Ystunned{x by the heavenly light!", victim, NULL, NULL, TO_ROOM );
    }
    if ( number_percent ( ) >= 98 && !IS_AFFECTED ( victim, AFF_BLIND ) )
    {
        ...
        act ( "$n is blinded by the heavenly light!", victim, NULL, NULL, TO_ROOM );
     }
...
MAGIC ( spell_fullheal )
{
    ...
    if ( IS_SHIELDED ( victim, SHD_HEMORRHAGE ) && check_dispel ( level, victim, skill_lookup ( "hemorrhage" ) ) )    
    {
...
MAGIC ( spell_ray_of_truth )
{
    ...
    act ( "$n raises $s hand, and a blinding ray of light shoots forth!", ch, NULL, ch, TO_ROOM );
...
MAGIC ( spell_summon )
{
    ...
    if ( strstr ( victim->in_room->area->builders, "Unlinked" ) )
    {
        send_to_char ( "You can not summon from the area where that is located.\n\r",
                       ch );
        return;
    }
...
MAGIC ( spell_ventriloquate )
{
    ...
    for ( vch = ch->in_room->people; vch != NULL; vch = vch->next_in_room )
    {
        if ( !is_name ( speaker, vch->name ) )
            send_to_char ( saves_spell ( level, vch, DAM_CHARM ) ? buf2 : buf1,
                           vch );
    }
...
MAGIC ( spell_acid_rain )
{
    ...
    act ( "{CW{ca{Cv{ce{Cs{g of {Ga{gc{Gi{gd{Gi{gc {Brain {gcalled by $n shower down upon {Ryou{g!{x", ch, NULL, victim, TO_VICT );
    act ( "{CW{ca{Cv{ce{Cs{g of {Ga{gc{Gi{gd{Gi{gc {Brain {gshower down upon {R$N{g!{x", ch, NULL, victim, TO_CHAR );
    i = 8;
    while ( i > 0 )
    {
...
MAGIC ( spell_elemental_fury )
{
    ...
    act ( "$n unleashes the {Gf{gu{Yr{Ry{x of the {Re{Yl{Ge{Cm{Be{Gn{Yt{Rs{x!", ch, NULL, NULL, TO_ROOM );
    send_to_char ( "You unleash the {Gf{gu{Yr{Ry{x of the {Re{Yl{Ge{Cm{Be{Gn{Yt{Rs{x!\n\r", ch );
