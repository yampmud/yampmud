      ...
      value < 0)
  {
    if (ch->pcdata->security != 0)
    {
      sprintf(buf, "Security is 0-%d.\n\r",
              IS_TRUSTED(ch,
                         IMPLEMENTOR) ? MAX_SECURITY : ch->pcdata->security);
      send_to_char(buf, ch);
    }
    else
...
REDIT(redit_show)
{
  ...
  for (door = 0; door < MAX_DIR; door++)
  {
    ...
    if ((pexit = pRoom->exit[door]))
    {
      ...
      sprintf(buf, "-%-5s to [%ld] Key: [%ld] ", capitalize(dir_name[door]), pexit->u1.to_room ? pexit->u1.to_room->vnum : 0, /* ROM 
                                                                                                                                 OLC 
                                                                                                                               */
              pexit->key);
      strcat(buf1, buf);
  ...
  switch (obj->item_type)
  {
    ...
    case ITEM_CONTAINER:
      sprintf(buf,
              ...
              "[v1] Flags:      [%s]\n\r" "[v2] Key:     %s [%ld]\n\r"
              "[v3] Capacity    [%ld]\n\r" "[v4] Weight Mult [%ld]\n\r",
              obj->value[0], flag_string(container_flags,
                                         obj->value[1]),
...
OEDIT(oedit_addapply)
{
  ...
  send_to_char("Use addaffect [immune/resist/vuln] <imm/resist/vuln>.\n\r",
               ch);
  return false;
...
MEDIT(medit_act2)               /* Moved out of medit() due to naming conflicts 
                                   -- Hugin */
{
  MOB_INDEX_DATA *pMob;
  int value;
...
MEDIT(medit_autoset)
{
  ...
  bonus =
    (pMob->level / 40 + 1) * ((pMob->level * 10) + (pMob->level / 10)) + 1;
  size = pMob->level / 3;
  if (size < 2)
    size = 2;
...
MEDIT(medit_autohard)
{
  ...
  medit_damdice(ch, temp);
  bonus =
    (pMob->level / 40 + 10) * ((pMob->level * 10) + (pMob->level / 10)) + 1;
  size = pMob->level / 3;
  if (size < 2)
    size = 2;
  size++;
...
MEDIT(medit_autoeasy)
{
  ...
  medit_damdice(ch, temp);
  bonus =
    (pMob->level / 59 + 1) * ((pMob->level * 9) + (pMob->level / 11)) + 1;
  size = pMob->level / 3;
  if (size < 2)
    size = 2;
...
MEDIT(medit_delete)
{
  ...
  char buf[MSL];
  int index, count, iHash, i;
  int dobj[100];                /* I highly doubt one mobile will have 100
                                   unique objectresets */
  bool foundmob = false;
  bool exist = false;
  bool foundobj = false;
