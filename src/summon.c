...
//    OBJ_DATA *corpse_item_next;
    ...
    if ( !found ) // Damn we couldn't find that one.
    {
        while ( i < MAX_SUMMON )
        {
            ...
            {
                sprintf ( spell_temp_buf, "%d: '%s' %s\n\r", summon_table[i].level_needed, skill_table[sn].name, summon_table[i].name );
    ...
    {
        sprintf ( lev_buf, "You must be level %d to cast %s %s.\n\r", summon_table[summon_id].level_needed, skill_table[sn].name, summon_table[summon_id].name );
    ...
    if ( summon >= 2 )
    {
        send_to_char ( "You can not have any more summoned beings in your control.\n\r", ch );
        return;
    }
...
*/
    ...
    if ( ( pMobIndex = get_mob_index ( vnum ) ) == NULL )
    {
        ...
        send_to_char ( "Sorry, an error has occurred. Please write a note to 'immortal'.\n\r", ch );
    ...
    sprintf ( act_buf, "%s\n\r{w%s has the mark of %s on its forehead.\n\r", victim->description, victim->short_descr, ch->name );
    ...
    victim->max_hit =	( ch->level *	summon_table[summon_id].hp_usage	+ dice ( 36, 36 ) 	+ 1 );
    victim->max_mana =	( ( ch->level *	summon_table[summon_id].mana_usage	+ dice ( 36, 36 ) ) + 1 / 2 );
    victim->max_move =	( ch->level *	summon_table[summon_id].move_usage	+ dice ( 36, 36 ) 	+ 1 );
    ...
    act ( "$n summons $N and channels their energy into it.",	ch, NULL, victim, TO_ROOM );
    act ( "You summon $N and channel your energy into it.",		ch, NULL, victim, TO_CHAR );
