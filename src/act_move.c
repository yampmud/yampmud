        ...
        ch->level >= 202) || IS_IMMORTAL(ch)))
  {
    ...
    if (item && obj != NULL)
      if ((!IS_SET(obj->wear_flags, ITEM_TAKE) ||
           IS_SET(obj->in_room->room_flags, ROOM_NODRAG)) && !IS_IMMORTAL(ch))
      {
    ...
    if (item && obj != NULL)
      if (obj->weight > (2 * can_carry_w(ch)) && !IS_IMMORTAL(ch))
      {
    ...
    if (item)
    {
      ...
      send_to_char(buf, ch);
      act("$n drags $p out of the room with $m.", ch, obj, NULL, TO_ROOM);
    ...
    else
    {
      ...
      if (IS_AFFECTED(victim, AFF_ENTANGLE))
      {
        ...
        {
          act
            ("$n tries to drag your from the room, but the web of mana holds.",
             ch, NULL, victim, TO_VICT);
          act("$n tries to drag $N from the room, but the web of mana holds.",
              ch, NULL, victim, TO_NOTVICT);
          act("You try to drag $N from the room, but the web of mana holds.",
      ...
      if (IS_SET(ch->in_room->room_flags, ROOM_NODRAG) && !IS_IMMORTAL(ch))
      {
...
void move_char(CHAR_DATA * ch, int door, bool follow, bool quiet)
{
  ...
  if (IS_SET(pexit->exit_info, EX_CLOSED) &&
      (!IS_AFFECTED(ch, AFF_PASS_DOOR) ||
       IS_SET(pexit->exit_info, EX_NOPASS)) && !IS_TRUSTED(ch, KNIGHT))
  {
    if (!quiet)
  ...
  if (obj != NULL)
  {
    if (!IS_SET(obj->item_type, ITEM_FURNITURE) ||
        (!IS_SET(obj->value[2], REST_ON) &&
         !IS_SET(obj->value[2], REST_IN) && !IS_SET(obj->value[2], REST_AT)))
    {
...
CH_CMD(do_swalk)
{
  ...
  else
  {
    if (((ch->hit * 100) / ch->max_hit) <= 90)
      ...
      printf_to_char(ch, "You attempt to meld into the shadows but fail.");
...
CH_CMD(do_recall)
{
  ...
  {
    if (IS_SET(ch->in_room->room_flags, ROOM_RECALL_HERE) || IS_IMMORTAL(ch))
    {
