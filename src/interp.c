...
const struct cmd_type cmd_table[] = {
  ...
  {"backup", do_backup, POS_DEAD, 0, 1, LOG_NORMAL, 1},
...
CH_CMD(do_commands)
{
  ...
  for (cmd = 0; cmd_table[cmd].name[0] != '\0'; cmd++)
  {
    if (cmd_table[cmd].level <= LEVEL_ANCIENT && cmd_table[cmd].show &&
        strlen(cmd_table[cmd].name) > 1)
    {
      sprintf(buf, "%s%-12s",
              linetog == 0 ? "{W" : linetog == 1 ? "{G" : "{g",
              cmd_table[cmd].name);
      send_to_char(buf, ch);
  ...
  for (lvl = MAX_LEVEL; lvl >= LEVEL_IMMORTAL && lvl <= MAX_LEVEL; lvl--)
  {
    ...
    for (cmd = 0; cmd_table[cmd].name[0] != '\0'; cmd++)
    {
      if ((cmd_table[cmd].level >= LEVEL_IMMORTAL
           ...
           && cmd_table[cmd].level == lvl && cmd_table[cmd].show))
      {
        if (lvl != lvl_old)
        {
          if (col % 6 != 0)
