            ...
            else
            {
                mana =
                    UMAX ( skill_table[sn].min_mana,
                           100 / ( 2 + ch->level - lev ) );
                sprintf ( buf, "{W%-18s {g %3d {xmana  ", skill_table[sn].name,
                          mana );
            }
            ...
            if ( lev == level )
            {
                ...
                else
                {
                    if ( level > lastlevel )
                    {
                        ...
                        sprintf ( buf, "{c%-3d{C: %s%-19s %s {w| ", level, ctoggle ? "{W" : "{w", skill_table[sn].name, learnbuf );
                        send_to_char ( buf, ch );
                        toggle = 0;
                    }
                    else
                    {
                        sprintf ( buf, "     %s%-19s %s {w| ", ctoggle ? "{W" : "{w", skill_table[sn].name, learnbuf );
                        send_to_char ( buf, ch );
