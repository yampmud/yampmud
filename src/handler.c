...
int pcrace_lookup(const char *name)
{
  ...
  {
    if (!str_prefix(name, pc_race_table[race].name))
      return race;
  }
...
char *weapon_name(int weapon_type)
{
  ...
  for (type = 0; type < MAX_WEAPON; type++)
...
char *wiznet_find(long flag)
{
  ...
  for (find = 0; wiznet_table[find].flag; find++)
  {
    if (wiznet_table[find].flag == flag)
    {
      buf = wiznet_table[find].name;
    }
  }
...
int check_immune(CHAR_DATA * ch, int dam_type)
{
  ...
  else if (dam_type <= 11)      /* natural attack */
  {
    if (IS_SET(ch->imm_flags, IMM_NATURE))
      def = IS_IMMUNE;
    else if (IS_SET(ch->res_flags, RES_NATURE))
      def = IS_RESISTANT;
    else if (IS_SET(ch->vuln_flags, VULN_NATURE))
      def = IS_VULNERABLE;
  }
  else                          /* magical attack */
  {
...
int get_skill(CHAR_DATA * ch, int sn)
{
  ...
  else if (!IS_NPC(ch))
  {
    ...
    {
      if (IS_SHIELDED(ch, SHD_CONCENTRATION))
      {
        skill = (ch->pcdata->learned[sn] + (ch->pcdata->learned[sn] / 10));
      }
      else
      {
    ...
    if (skill_table[sn].spell_fun != spell_null)
      ...
      skill = ch->level / 2;
             ...
             (IS_SET(ch->off_flags, OFF_DISARM) ||
              IS_SET(ch->act, ACT_WARRIOR) ||
              IS_SET(ch->act, ACT_THIEF) || IS_SET(ch->act, ACT_VAMPIRE)))
...
void extract_char(CHAR_DATA * ch, bool fPull)
{
  ...
  if (!fPull)
  {
    if ((clan_table[ch->clan].hall == ROOM_VNUM_ALTAR) && ch->alignment < 0)
...
char *shield_bit_name(int vector)
{
  ...
  if (vector & SHD_MANA)
    strcat(buf, " mana_shiled");

  if (vector & SHD_SWALK)
    strcat(buf, " swalk");
  if (vector & SHD_WARD)
    strcat(buf, " ward");
  if (vector & SHD_DIVINE_PROTECTION)
    strcat(buf, " divine_prot");
  if (vector & SHD_VANISH)
    strcat(buf, " vanish");
  if (vector & SHD_STONE_SKIN)
    strcat(buf, " stone_skin");
  if (vector & SHD_BLESS)
    strcat(buf, " bless");
  if (vector & SHD_FRENZY)
    strcat(buf, " frenzy");
  if (vector & SHD_LIFE)
    strcat(buf, " lifeforce");
  if (vector & SHD_MISTBLEND)
    strcat(buf, " mistblend");
  if (vector & SHD_INSPIRE)
    strcat(buf, " inspire");
  if (vector & SHD_RETREAT)
    strcat(buf, " retreat");
  if (vector & SHD_SILENCE)
    strcat(buf, " silence");
  if (vector & SHD_CONCENTRATION)
    strcat(buf, " concentration");
  if (vector & SHD_HEMORRHAGE)
    strcat(buf, " hemorrhage");

  if (vector & SHD_LASTRITES)
    strcat(buf, " last_rites");
  if (vector & (bb))
    strcat(buf, " invalid: bb");
  if (vector & (cc))
    strcat(buf, " invalid: cc");
  if (vector & (dd))
    strcat(buf, " invalid: dd");
  if (vector & (ee))
